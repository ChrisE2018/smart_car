...
class BasicIndexMatrix<Rows, 1, value_type> : public BasicMatrix<Rows, 1, value_type>
{
    ...
    public:
        ...
        TMatrix<Rows, Rows, value_type> diag (void) const
        {
            ...
            for (uint32_t i = 0; i < Rows; i++)
                d.element(i, i, BaseType::mData[i]);
            return d;
        }
...
class TMatrix<4, 1, value_type> : public BasicIndexMatrix<4, 1, value_type>
{
    ...
    public:
        ...
        TMatrix (value_type a0, value_type a1, value_type a2, value_type a3) : BaseType(
                TMatrixDummy())
        {
            BaseType::mData[0] = a0;
            BaseType::mData[1] = a1;
...
class TMatrix<3, 1, value_type> : public BasicIndexMatrix<3, 1, value_type>
{
    ...
    public:
        ...
        TMatrix<3, 1, value_type> cross (const TMatrix<3, 1, value_type> &v) const
        {
            ...
            return TMatrix<3, 1, value_type>(u[1] * v[2] - u[2] * v[1], u[2] * v[0] - u[0] * v[2],
                    u[0] * v[1] - u[1] * v[0]);
        }
};
...
class TMatrix<1, 1, value_type> : public BasicIndexMatrix<1, 1, value_type>
{
    ...
    public:
        ...
        explicit TMatrix (value_type a0) : BaseType(TMatrixDummy())
        { // don't initialize
            BaseType::mData[0] = a0;
        }
...
class BasicMatrix
{
    ...
    public:
        ...
        {
            STATIC_ASSERT((R + RowRangeSize <= Rows) && (C + ColRangeSize <= Cols));
        ...
        TMatrix<Rows, RhsCols, value_type> operator* (
                const TMatrix<Cols, RhsCols, value_type> &rhs) const
        {
         ...
         */
        void svdPartial (TMatrix<Rows, Cols> *U, TMatrix<Cols, 1> *S, TMatrix<Cols, Cols> *V) const
        {
        ...
        value_type* row (uint32_t i)
        {
            return &mData[i * Cols];
        }
        const value_type* row (uint32_t i) const
        {
            return &mData[i * Cols];
        }
    ...
    private:
        ...
        friend TMatrix<Rows2, Cols2, value_type2> operator* (double s,
                const TMatrix<Rows2, Cols2, value_type2> &m);

        template<uint16_t Rows2, uint16_t Cols2, typename value_type2>
        friend std::ostream& operator<< (std::ostream &os,
                const TMatrix<Rows2, Cols2, value_type2> &rhs);
